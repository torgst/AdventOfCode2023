...
const data: MapList<MapData[]> & {
  seeds?: { start: number; length: number }[];
} = {};
...
for (const match of matches) {
  if (match.name === "seeds") {
    ...
    if (seedranges.length % 2 !== 0) throw "Not an even number of ranges";
    data.seeds = [];
    for (let i = 0; i < seedranges.length / 2; i++) {
      data.seeds.push({
        start: seedranges[i * 2]!,
        length: seedranges[i * 2 + 1]!,
      });
    }
    logJson({ seeds: data.seeds });
...
for (const seedRange of data.seeds!) {
  for (let i = 0; i < seedRange.length; i++) {
    const location = getLocationForSeed(seedRange.start + i, finalMaps);
    if (minLocation === undefined || location < minLocation) {
      minLocation = location;
    }
  }
}
logString("Result: " + minLocation);
